...
Vector InverseDynamicsSolver::solve(const SimTK::State &s, const SimTK::Vector &udot)
{
    ...
    const Vector &appliedMobilityForces = getModel().getMultibodySystem().getMobilityForces(s, SimTK::Stage::Dynamics);
        
    // Get all applied body forces like those from contact
    const SimTK::Vector_<SimTK::SpatialVec>& appliedBodyForces = getModel().getMultibodySystem().getRigidBodyForces(s, SimTK::Stage::Dynamics);
...
void InverseDynamicsSolver::solve(SimTK::State &s, const FunctionSet &Qs, const SimTK::Array_<double> &times, SimTK::Array_<Vector> &genForceTrajectory)
{
...
void InverseDynamicsSolver::solve(SimTK::State& s, const FunctionSet& Qs,
        ...
        SimTK::Array_<Vector>& genForceTrajectory) {
    int nCoords = getModel().getNumCoordinates();
    int nt = times.size();
